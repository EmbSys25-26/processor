    ...
    always @(posedge i_clkSystem) begin
        if (i_rst) begin
            ...
            if (i_ascii_valid) begin
                ...
                // 1. ENTER (0x0D)
                if (i_ascii_code == 8'h0D) begin
                    _char_we <= 1'b0; // nao escrever "Enter" no ecra
                    if (_auto_inc) begin
                        // se ja estiver na ultima linha volta ao topo (0)
                        if (_char_addr >= 12'd2320)
                            _char_addr <= 12'd0;
                        // senao avanca os espaços que faltam até à próxima linha (múltiplo de 80)
                        else
                            _char_addr <= _char_addr + (12'd80 - (_char_addr % 12'd80));
                    end
                end
                
                // 2. BACKSPACE (0x08)
                else if (i_ascii_code == 8'h08) begin
                    // apenas recua se não estiver já na posição 0 do ecrã
                    if (_char_addr > 12'd0) begin
                        _char_waddr <= _char_addr - 12'd1; // aponta para a caixa anterior
                        _char_wdata <= 8'h20;              // escreve um "espaço" (0x20) para limpar a letra
                        _char_we    <= 1'b1;               // ativa a escrita
                        
                        if (_auto_inc)
                            _char_addr <= _char_addr - 12'd1; // atualiza o cursor para trás
                    end else begin
                        _char_we <= 1'b0; // ja está no início, não faz nada
                    end
                end
                
                // 3. LETRA/NÚMERO NORMAL
                else begin
                    _char_wdata <= i_ascii_code;
                    _char_waddr <= _char_addr;  // captura ANTES do auto-inc
                    _char_we    <= 1'b1;
                    if (_auto_inc)
                        _char_addr <= (_char_addr < 12'd2399) ? _char_addr + 12'd1 : 12'd0;
                end
