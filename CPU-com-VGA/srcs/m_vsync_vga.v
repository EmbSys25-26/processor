 ...
 always @(posedge i_clk) begin
        if (i_rst || ~i_enVGA) begin
            ...
            case (_state)
                ...
                // ---- Só modo imagem usa este estado ----
                // No modo texto entra aqui no reset mas sai imediatamente
                // pois não há tuser — avança logo para VISIBLE
                WAIT_SOF: begin
                    _vsync <= 1'b1;
                    if (i_mode_switch) begin
                        // Modo texto: não há VDMA, avança directamente
                        _state       <= VISIBLE;
                        _lineCounter <= 9'b0;
                    end else begin
                        // Modo imagem: aguarda SOF do VDMA
