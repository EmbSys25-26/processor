...
`ifdef SIM
    // Em simulação ambos os clocks são o mesmo - timing VGA não é verificado
    wire i_clkk  = i_clk;
    wire _clkVGA = i_clk;
`else
    wire i_clkk;
    wire _clkVGA;   // ← NOVO: 25.175 MHz para o pixel clock VGA
    wire locked;
 
    // O clk_wiz_0 precisa de ser reconfigurado no Vivado para ter clk_out2:
    //   clk_out1 → clock do sistema (ex: 100 MHz)
    //   clk_out2 → 25.175 MHz (VGA pixel clock)
    clk_wiz_0 clk_gen (
        .clk_out1 (i_clkk),
        .clk_out2 (_clkVGA),   // ← NOVO output do IP
        .reset    (1'b0),
        .locked   (locked),
        .clk_in1  (i_clk)
    );
...
//    wire _int_en_cpu;
//    wire _iret_detected;
    (* mark_debug = "true" *) wire _irq_take;
    (* mark_debug = "true" *) wire [15:0] _irq_vector;
    (* mark_debug = "true" *) wire _in_irq;
    (* mark_debug = "true" *) wire _int_en_cpu;
    (* mark_debug = "true" *) wire _iret_detected;
