 ...
 ************************************************************************************/
    ...
    wire _id_exec_valid;                // Instruction should be dispatched to EX (not IMM/CLI/STI pseudo-ops)
    wire [15:0] _id_pc;
    ...
    wire _ex_fire;                      // EX stage commits instruction
    ...
    // EX Branch Result
    wire _ex_branch_take;
    wire [15:0] _ex_branch_target_out;
    ...
    // Branch + BPU signals inside EX
    wire _idex_is_bx;
    wire [3:0] _idex_cond;
    wire [15:0] _idex_branch_target;
    wire _idex_pred_taken;
    wire [15:0] _idex_pred_target;
    wire [`GHR_W-1:0] _idex_lookup_ghr;
 ...
 ************************************************************************************/
    ...
    // _branch_take_commit: a branch/jump is actually committed from EX.
    assign _branch_take_commit = _ex_branch_take & _ex_fire;

    // Branch resolution and redirect policy in EX:
    // - JAL always redirects unconditionally using ALU output
    // - BX redirects only on misprediction evaluated in EX
    assign _bx_resolve = _ex_fire & _idex_is_bx;
    ...
    /* A mispredict happens in the following cases:
       ...
       3) Predicted taken to wrong target
    */
    assign _bx_mispredict = _bx_resolve &
                 ((_idex_pred_taken != _ex_branch_take) |
                 (_ex_branch_take & (_idex_pred_target != _ex_branch_target_out)));
                 
    assign _jal_redirect = _ex_fire & _idex_is_jal;
    assign _redirect = _jal_redirect | _bx_mispredict;
    
    // Redirect PC now comes explicitly from EX evaluation
    assign _redirect_pc = _ex_branch_take ? _ex_branch_target_out : (_idex_pc + 16'h0002);
 ...
 ************************************************************************************/
    pipe_id_ex u_pipe_id_ex (
        ...
        // Pass branch + BPU parameters cleanly to EX
        .i_is_bx(_id_is_bx),
        .i_cond(_id_cond),
        .i_branch_target(_id_branch_target),
        .i_pred_taken(_ifid_pred_taken),
        .i_pred_target(_ifid_pred_target),
        .i_lookup_ghr(_update_ghr),
        ...
        .o_is_bx(_idex_is_bx),
        .o_cond(_idex_cond),
        .o_branch_target(_idex_branch_target),
        .o_pred_taken(_idex_pred_taken),
        .o_pred_target(_idex_pred_target),
        .o_lookup_ghr(_idex_lookup_ghr)
    );
    ...
    ex_stage u_ex_stage (
        ...
        // Inputs from ID/EX register for Branch Resolution
        .i_is_bx(_idex_is_bx),
        .i_cond(_idex_cond),
        .i_branch_target(_idex_branch_target),
        ...
        // Evaluated Branch Results
        .o_branch_take(_ex_branch_take),
        .o_branch_target_out(_ex_branch_target_out)
    );
 ...
 ************************************************************************************/
    ...
    // update on resolved conditional branch exclusively from EX stage now.
    bpu u_bpu (
        .i_clk(i_clk),
        .i_rst(i_rst),
        .i_lookup_pc(_pc),
        .i_update_en(USE_DBP & _bx_resolve),
        .i_update_pc(_idex_pc),
        .i_update_taken(_ex_branch_take),
        .i_update_target(_ex_branch_target_out),
        .i_update_ghr(_idex_lookup_ghr),
        .i_br_uncond(_idex_is_bx && ((_idex_cond & 4'b1110) == 4'h0)), // Derived from IDEX bundle
